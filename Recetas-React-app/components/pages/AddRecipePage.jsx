...
export default function AddRecipePage({ navigation, route }) {
  ...
  useEffect(() => {
    if (isEditing && editingRecipe) {
      ...
      console.log(JSON.stringify(editingRecipe, null, 2));

      setForm({
        titulo: editingRecipe.titulo || "",
        descripcion: editingRecipe.descripcion || "",
        tiempo_coccion: editingRecipe.tiempo_coccion
          ? String(editingRecipe.tiempo_coccion)
          : "",
        ...
        porciones: editingRecipe.porciones
          ? String(editingRecipe.porciones)
          : "2",
        is_public:
          editingRecipe.is_public !== undefined
            ? editingRecipe.is_public
            : true,

        ingredients: editingRecipe.ingredientes?.length
          ? editingRecipe.ingredientes
          : [{ nombre: "", cantidad: "" }],

        steps: editingRecipe.pasos?.length
          ? editingRecipe.pasos
          : [{ descripcion: "" }],
  ...
  const handleSubmit = async () => {
    ...
    if (!user?.id) {
      Alert.alert(
        ...
        "Necesitas iniciar sesión para crear una receta.",
      );
    ...
    console.log("Ingredientes recibidos:", form.ingredients);
    console.log("Pasos recibidos:", form.steps);

    const ingredients = form.ingredients.filter((item) => {
      console.log("Ingrediente individual:", item);

      return (
        item && typeof item.nombre === "string" && item.nombre.trim().length > 0
      );
    });

    const steps = form.steps.filter((item) => {
      console.log("Paso individual:", item);
    ...
    console.log("Ingredientes filtrados:", ingredients);
    console.log("Pasos filtrados:", steps);
    ...
    try {
      const recipeData = {
        ...
        tiempo_coccion: Number.parseInt(form.tiempo_coccion, 10) || 0,

        dificultad: form.dificultad || "Easy",

        calorias: Number.parseInt(form.calorias, 10) || 0,

        porciones: Number.parseInt(form.porciones, 10) || 1,
        ...
        groupId: form.wantsGroup && form.groupId ? form.groupId : null,
      };

      console.log("recipeData final:", recipeData);
      ...
      if (isEditing) {
        ...
        console.log("Resultado updateRecipe:", result);
        ...
        console.log("Resultado createRecipe:", result);
  ...
  return (
    ...
    >
      ...
      <Input
        placeholder="Ej. Pastel de Manzana"
        value={form.titulo}
      ...
      <View style={styles.toggleCard}>
        ...
        <View style={styles.toggleRow}>
          <TouchableOpacity
            style={[
              styles.togglePill,
              form.is_public && styles.togglePillActive,
            ]}
          ...
          >
            <Text
              style={[
                styles.toggleText,
                form.is_public && styles.toggleTextActive,
              ]}
            >
              Pública
            </Text>
      ...
      <View style={styles.toggleCard}>
        ...
        <View style={styles.toggleRow}>
          <TouchableOpacity
            style={[
              styles.togglePill,
              !form.wantsGroup && styles.togglePillActive,
            ]}
          ...
          >
            <Text
              style={[
                styles.toggleText,
                !form.wantsGroup && styles.toggleTextActive,
              ]}
            >
              No
            </Text>
        ...
        {form.wantsGroup ? (
          groups.length > 0 ? (
            ...
            <Text style={styles.helperText}>
              No tienes grupos creados. Crea un grupo primero en la pestaña
              Grupos.
            </Text>
          )
        ) : (
          <Text style={styles.helperText}>
            La receta se guardará sin grupo.
          </Text>
        )}
      </View>
